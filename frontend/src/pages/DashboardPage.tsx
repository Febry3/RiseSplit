...
import { ArrowLeft, Share2, ShieldCheck, TrendingUp, Wallet } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const handleConnectWallet = async (): Promise<void> => {
    ...
    try {
      ...
      if (
        freighterPassphrase &&
        appPassphrase &&
        freighterPassphrase !== appPassphrase
      ) {
  ...
  return (
    <div className="page-shell relative mx-auto w-full max-w-7xl px-4 pb-12 pt-6 font-body text-white sm:px-6 lg:px-8">
      ...
      <header className="site-nav glass-card sticky top-4 z-20 mb-8 animate-fade-up p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="font-display text-lg font-semibold tracking-tight">Ghost-Author Dashboard</p>
            <p className="text-sm text-white/65">Creator Studio + Payer Interface + Analytics</p>
          </div>

          <div className="flex items-center gap-3">
            <Link className="ghost-outline-button inline-flex items-center" to="/">
            ...
            {walletAddress ? (
              <div className="glass-card flex items-center gap-3 px-3 py-2">
                <span className="badge border-mint/50 bg-mint/20 text-mint">Live</span>
                <div>
                  <p className="text-sm font-semibold">{shortAddress(walletAddress)}</p>
                  <p className="text-xs text-white/60">{Number(xlmBalance).toFixed(2)} XLM</p>
                </div>
              </div>
            ) : null}
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-3">
        <article className="stat-card glass-card p-4">
        ...
        <article className="stat-card glass-card p-4">
          ...
          <p className="mt-2 text-sm font-medium">{formatDate(lastPaymentAt)}</p>
          {isRefreshing ? (
      ...
      <section className="table-shell glass-card mt-6 overflow-hidden">
